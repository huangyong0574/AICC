...
|---|---|---|---|
| **Feynman Warmup** | Dynamic 3 role-based questions——链接客户决策层三角色：CEO(biz)/CTO(dev)/业务运营负责人(internal)，role key 保持旧值兼容已存笔记；事实锚定：注入该认知点的雷达 insight（tagline/corePrinciple/whyMatters）作唯一事实依据、禁止编造；手建概念回退纯标题 | Done | `callFeynmanWarmup(raw, cfg, material?)` + `loadInsightById()` |
...
> **Phase 2.1（数据唯一来源铁律）**：所有用户数据必须落 vault、不只存浏览器。费曼笔记 write-through 进 `vault/AICC/notes/<id>.json`（JSON 无损）、草稿进 `drafts/`（`/api/vault/{notes,note,drafts,draft}`）；`hydrateFromVault()` 恢复笔记 + 一次性回灌既有笔记（自愈）。仅纯 UI 偏好（`aicc-theme`）留浏览器；派生/缓存（deep-plan/topics/feynman-graph）可重建。
> **Phase 2.2（文章双向同步 A 档）**：写作台/文章页**打开即从 vault 读最新**（`fetchArticle(slug)` + `cleanArticleBody()` 剥 frontmatter + 机器「## 融合」块）→ 在 Obsidian 改完正文、网页重开即见。读取路径直接 try `fetchArticle`（自带探测、无 Gateway/无此文则回退本地/静态），不依赖与 render 并发的 `initVault`。B 档（开着页面实时轮询刷新）后续按需。
> **Phase 2.3（写作归 Obsidian）**：成文写作迁出 App、归 Obsidian。`POST /api/vault/article-obsidian`（`ensureArticleForObsidian`）：文章存在→返回 `obsidian://open?path=<绝对路径>`；不存在→按骨架建 `.md`（frontmatter + 选题钩子 + 已闭环概念素材脚手架 + `## 融合 [[concept]]`）再返回链接。创作写作台「✍ 在 Obsidian 写」= 建骨架 + 跳 Obsidian；文章页「✍ 在 Obsidian 编辑」取代旧 EditorPage 入口（`/editor` 降级为直达「粘贴导入」工具）。发布以 vault 最新正文为准（`fetchArticle`+`cleanArticleBody`，回退本地编辑区）。
...
> **Phase 4（雷达消费 vault）**：Gateway（`server/radar.mjs` + `/api/radar/{index,week}`）直接读 `${VAULT_DIR}/AICC-Input/` 的 `{weekId}.json`（∪ 打包 `content/radar` 历史周）。前端 `radarData.ts` 优先走 `/api/radar/*`、无 Gateway 回退静态。**个人自用**：新周 JSON 落 AICC-Input 即多一期，免 `ingest→git→build→ECS`（该管线仅公开 ECS 阅读页仍需）。
...
interface Step1Route {        // 机制路由图（可选）：入口 → 关键判断节点 → 多分支；仅路由/分类/门控/决策类机制产出，纯线性算法省略
  entry: string;              // 入口节点（如：用户请求）
  gate: string;               // 关键判断/分流节点（如：安全分类器）
  branches: { tone: 'ok'|'warn'|'danger'; label: string; target: string; note: string }[]; // 2–3 分支（ok 放行/warn 降级/danger 拦截）
