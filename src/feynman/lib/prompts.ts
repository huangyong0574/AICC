...
export function buildStep1Prompt(question?: string): QaPrompt {
  ...
  "route": {"entry":"入口节点（如 用户请求）","gate":"关键判断/分流节点（如 安全分类器）","branches":[{"tone":"ok","label":"分支条件（如 常规·直接放行）","target":"目标结果（如 Mythos 5）","note":"一句说明（如 完全能力模型）"},{"tone":"warn","label":"另一走向（如 高危·静默路由）","target":"目标（如 Opus 4.8）","note":"说明（如 备用·风险更低）"}],"note":"底部补充说明（可选）"},
  "essence": "本质升华：一句话把这个机制提到系统/范式层面（如「把能力边界从静态训练阶段移到动态运行时，安全从提示词工程升级为一项系统工程」）。可选，无明显范式跃迁则省略。",
  "dims": ["关联工程维度1（如 能力分层）","维度2（如 场景识别）","维度3（如 审计日志）"],
  "glossaryTerms": [
...
route 字段仅当该概念存在「输入 → 关键判断节点 → 多条不同走向」的运行时机制（路由 / 分类 / 门控 / 降级 / 决策类）时才产出：2–3 条分支，tone 取 ok（放行·常规）/ warn（降级·谨慎）/ danger（拦截·高危）；纯线性算法（单一前向计算、无分流）则整个省略 route 字段，不要硬凑。
essence + dims 仅当该概念确有「从局部技巧上升为系统工程/新范式」的本质跃迁时才产出：essence 一句话点出范式转变，dims 给 3–5 个该范式牵动的工程维度（每个 ≤6 字）；若只是常规算法改进、无范式层面升华，则两者一并省略，不要硬凑。`,
  }
}
...
export function buildStep2Prompt(question?: string): QaPrompt {
  return {
    ...
    schema: `{
  "intro": "场景选择总述：一段话概括这个技术的业务适用边界，帮读者快速定位；把最关键的「边界判断句」用 **双星号** 加粗（不限字数）",
  "applicable": [
  ...
  "selectionCriteria": "一句引导句：概括如何判断自己的业务是否适用（如「三个条件同时满足，才适合引入」，≤40字）",
  "selectionConditions": ["条件1（≤8字，如 对外服务）", "条件2（如 风险高低混杂）", "条件3（如 有多档位模型）"],
...
applicable 2条，inapplicable 2条。fit 取值：excellent=完美适配，good=较适合，neutral=一般，poor=不太适合，unsuitable=不适用。selectionConditions 给 2-4 个「同时满足才适用」的简短判据（每条≤8字）；若该技术适用条件单一、无法拆出多条，可省略该字段。`,
  }
}
