import { Building2, Cpu, ExternalLink, Target } from 'lucide-react'
...
export function RadarCard({ insight, inPlan, onTogglePlan }: RadarCardProps) {
  const railBg = inPlan ? 'bg-[hsl(var(--mature)/0.06)]' : 'bg-secondary/50'
    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-[96px_1fr] min-h-full">
        ...
        <div className="p-6 sm:py-8 sm:px-9 sm:pb-7 min-w-0">
          ...
          <div
            className="text-[1rem] sm:text-[1.0625rem] font-normal text-foreground leading-[1.65] my-[18px] sm:mb-7 pl-[18px] border-l-2 border-foreground tracking-[-0.005em]"
          >
            {insight.tagline}
          </div>
          ...
          <div className="mt-[22px]">
            ...
            <p
              className="text-[0.9375rem] leading-[1.78] tracking-[0.005em]"
          ...
          <div className="flex items-center justify-between flex-wrap gap-3 mt-7 pt-5 border-t border-dashed border-border">
            ...
            <PlanToggle active={inPlan} onClick={onTogglePlan} />
          </div>
        </div>
      </div>
    </article>
