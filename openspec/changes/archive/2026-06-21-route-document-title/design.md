...
- **Non-Goals**：不引入 `react-helmet` 等三方库（与手写路由的零依赖风格一致）；不接管 article 标题（`ArticlePage` 已用真实文章标题设置）。
...
- `article` → **由 `ArticlePage` 用真实文章标题设置**（`<meta.title> — AICC`，兜底 slug）；本 effect 对 article 早退、不参与（实现期发现 ArticlePage 已设 `document.title`）
